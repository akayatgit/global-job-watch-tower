...
def lens_send_kpi_board(
    ...
    try:
        ...
        return json.dumps({'ok': ok, 'kind': 'kpi_board'})
...
def lens_send_pie_board(
    ...
    """Send a pie chart + legend drawn from EXACT items_json values (Pillow — not Grok).
    items_json: [{"label":"Role","value":64}, ...] from STAGEHAND only.
    Use for 'as pie chart' / legend / role breakdown asks."""
    try:
        ...
        return json.dumps({'ok': ok, 'kind': 'pie_board', 'slices': len(slices)})
    except Exception as e:
        return json.dumps({'ok': False, 'error': str(e)[:240]})
...
def lens_send_bar_board(
    ...
    """Send a bar chart from EXACT items_json (Pillow). Same schema as pie board."""
    try:
        ...
        return json.dumps({'ok': ok, 'kind': 'bar_board', 'bars': len(bars)})
    except Exception as e:
        return json.dumps({'ok': False, 'error': str(e)[:240]})
