...
| Magic pencil | After the original prompt + 14 stills land, Telegram shows **💥✏️ Twist**. That is our touch on the recreation — not a caption. Gemini (text-only second call) rewrites **every** timestamped beat: timing, emotion, context, imagination. **Final twisted prompt ≤ 3000 characters including spaces** — full essence, no filler; Gemini is told the cap, then a deterministic fit keeps every timestamp and the Style lock. Each cut JPEG goes through **text+image→image** (`REPLICATE_MODEL` / nano-banana-2 `image_input`, same camera and crop, twist visible) with a **series lock** (same look / palette / materials / lighting on every still; only the beat changes). Original prompt and original frames stay. Delivery: twisted prompt (`N/3000 chars`) + `twist-01-0.00s.jpg` documents. Stored as `twist_text` / `twist_prompt` / `twist_frames` (migration `b4e7c1a90d28`). Skip at intake still leaves the button; tapping it asks for the line if none was stored. |
