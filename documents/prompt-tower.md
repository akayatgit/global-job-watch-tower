...
| Pace | Worker writes `timings` JSON (`started_at`, per-step seconds, `total` / `twist_total`) on `reverse_prompts` (migration `d6a1c4e80b29`). Telegram shows one `⏱` line. Goal: cut the wait. After twisted stills commit, Telegram says Omni is rendering (2–6 min, two attempts). A stuck twist retries **Omni only**. |
