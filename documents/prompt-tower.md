...
| `/igtovid` · `/pintovid` [`url`] | **Reverse prompt** (§3b). **Public — every user.** Instant: `Now Send me the Instagram or Pinterest link.` Then **Whats the hook?** then **Select a Prompt Model…**. Finished clip carries Save + Images (4 then More) + Show prompt + Copy prompt + Twist. |
...
| Pace | Worker writes `timings` JSON (`started_at`, per-step seconds, `total` / `twist_total`) on `reverse_prompts` (migration `d6a1c4e80b29`). Telegram shows one `⏱` line. Goal: cut the wait. After twisted stills commit, **Omni is a new Celery + Replicate call** (Nano Banana must not own that process). Telegram says Omni is rendering (2–6 min, two attempts). A stuck twist retries **Omni only**. |
