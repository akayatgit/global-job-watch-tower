...
| Step | Law |
|---|---|
| Intake | `/igtovid` · `/pintovid` · `/pintovideo` · `/reverseprompt`. Instant one-liner: **Now Send me the Instagram or Pinterest link.** (no essay, not queued behind `/promptscan`). Then URL (or forwarded video) → **header title** → **magic-pencil twist** → **model buttons**. Footer is hardcoded. A stray direct `.mp4` in chat does **not** start a run unless he already typed the command. Cancel clears the wait. |
