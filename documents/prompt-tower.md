...
URL or forwarded clip
  ...
  → magic pencil: Gemini rewrites every beat · Nano Banana 2 Lite on each still
    · Seedance 2.0 motion-transfers from the **source clip** (`reference_videos`)
      + twist text + twisted stills (`reference_images`) — replaces Gemini Omni
  → Telegram: Seedance MP4 (`sendVideo`) + Images / Show / Copy + one `⏱` timing line
```

| Step | Law |
|---|---|
| Intake | `/igtovid` · `/pintovid` · `/pintovideo` · `/reverseprompt`. Instant: **Now Paste the link.** Then **Whats the hook?** then **Select a Prompt Model…** → **processing..** → **Workflow Started…**. Every reverse message/button acks within **1s** on the poll thread (hook → model picker immediately; otherwise static `…` then the real reply). Twist is not asked here. No essay. Not queued behind `/promptscan`. A stray direct `.mp4` in chat does **not** start a run unless he already typed the command. No Cancel / Retry buttons — paste a link anytime. |
...
| Magic pencil | Finished clip message is **Shall we twist the video?** with Save clip · Save reel · **Images** · Show prompt · Copy prompt · Twist. The prompt is never dumped. Show sends it; Copy sends a `.txt`. Twist starts the rewrite. Gemini (text-only second call) rewrites **every** timestamped beat — **3 tries**. If Gemini still misses (empty, short, no timestamps), **stamp the twist onto every original beat** and continue to stills + Seedance — do not fail the pass on rewrite alone. **Twisted prompt stays strictly under 3000 characters including spaces**. Then each cut JPEG goes through **text+image→image** on **Nano Banana 2 Lite**. Then **Seedance 2.0** (`bytedance/seedance-2.0` on Replicate) motion-transfers from the **source clip** (`reference_videos`) + twist text + twisted stills (`reference_images`). Telegram sends the Seedance MP4 as `sendVideo`. Twisted stills also wait behind **Images** (4 then More). Stills stay if Seedance fails (`twist_video_error`). A later fail is plain text — no Retry / Cancel buttons; paste a link or tap Twist again anytime. Original prompt and original frames stay. Stored as `twist_text` / `twist_prompt` / `twist_frames` / `twist_video_*` (migration `c5f8a2d19e37`). A leftover Gemini tap after start **resumes** the last reverse — it does not re-ask the link. |
| Pace | Worker writes `timings` JSON on `reverse_prompts`. After twisted stills, Telegram offers **Images** (4 then More) and **▶ Start the Twist**. That tap is the Seedance Replicate call — never auto-start after Nano Banana. |
...
| `GET /reverse` · `GET /reverse/{id}` | Recent rows / one row: `queued \| downloading \| describing \| composing \| done \| failed`, clip + prompt + `ref_frames` + twist + Seedance video + reel + `timings` |
...
| `PROMPT_TWIST_VIDEO_MODEL` | `bytedance/seedance-2.0` | After twisted stills: Seedance motion-transfer (source clip + stills). Empty env falls back here. |
