...
| Normalize | `app/prompts/normalize.py` | Deterministic: a prompt must be ≥180 chars, touch ≥3 vocabulary families, include camera-or-lighting **and** product-or-motion. Heuristic 0–100 from literal structure (families, length, numbers, sentence flow). Category (perfume, skincare, beverage…) and model hint (veo/kling/sora…) from literal words only. |
