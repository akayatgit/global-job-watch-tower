...
| **Public Telegram** | **Every user** — `/igtovid` · `/pintovid` · paste an Instagram / Pinterest link. Copy is one line: `Now Send me the Instagram or Pinterest link.` → `Whats the hook?` → `Select a Prompt Model…` → `Workflow Started…`. Finished clip: Save + Show prompt + Copy prompt + Twist. |
...
| `/igtovid` · `/pintovid` [`url`] | **Reverse prompt** (§3b). **Public — every user.** Instant: `Now Send me the Instagram or Pinterest link.` Then **Whats the hook?** then **Select a Prompt Model…**. Finished clip carries Save + Show prompt + Copy prompt + Twist. |
...
URL or forwarded clip
  ...
  → Telegram: original clip (`sendVideo`) + reel (`sendVideo`) → Save / Show / Copy / Twist
  → cut frames as one `frames-N.zip` (download all)
  ...
  → Telegram: Omni MP4 (`sendVideo`) + `twist-frames-N.zip` + Show / Copy
```
...
| Magic pencil | Finished clip message is **Shall we twist the video?** with Save clip · Save reel · Show prompt · Copy prompt · Twist. The prompt is never dumped. Show sends it; Copy sends a `.txt`. Twist starts the rewrite. Gemini (text-only second call) rewrites **every** timestamped beat. **Twisted prompt stays strictly under 3000 characters including spaces**. Then each cut JPEG goes through **text+image→image** on **Nano Banana 2 Lite**. Then **Gemini Omni 1.1** (`google/gemini-omni-1.1` on Replicate) motion-transfers the original clip onto those stills. Telegram sends the Omni MP4 as `sendVideo` (the final twisted video) plus one `twist-frames-N.zip`. Stills stay if Omni fails (`twist_video_error`). Original prompt and original frames stay. Stored as `twist_text` / `twist_prompt` / `twist_frames` / `twist_video_*` (migration `c5f8a2d19e37`). |
...
| `GET /reverse` · `GET /reverse/{id}` | Recent rows / one row: `queued \| downloading \| describing \| composing \| done \| failed`, clip + prompt + `ref_frames` + twist + Omni video + reel |
...
- `reverse_prompts` (migration `d9e5a1c47f02`) — platform (`instagram \| pinterest \| direct \| upload`), source/media URL, stored clip, duration, **header_title**, **vision_engine** (`gemini \| astra \| fable`, migration `f2c4d6e8a910`), **keyword + prompt_text** (verbatim from the chosen model; `cuts` stripped), **ref_frames / ref_error** (migration `a3c9e1b72d04` — ~14 cut-reference JPEGs), **twist_text / twist_prompt / twist_frames / twist_status** (migration `b4e7c1a90d28` — magic pencil), **twist_video_key / twist_video_url / twist_video_error** (migration `c5f8a2d19e37` — Gemini Omni motion-transfer MP4), optional `prompt_id` into the catalogue, reel key/url/error, status (`queued \| downloading \| describing \| composing \| done \| failed`).
