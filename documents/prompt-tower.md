...
| `pt:photo` | "Prompt #N + your product photo are paired" → `✅ Make video`. |
...
| `/igtovid` · `/pintovid` [`url`] | **Reverse prompt** (§3b). **Public — every user.** Instant: `Now Paste the link.` Then **Whats the hook?** then **Select a Prompt Model…**. Finished clip carries Save + Images (4 then More) + Show prompt + Copy prompt + Twist. |
...
URL or forwarded clip
  ...
  → Telegram: original clip (`sendVideo`) + reel (`sendVideo`) → Save / Images / Show / Copy / Twist
  → cut frames only on **🖼 Images** (4 JPEGs, then More images ▸) — never auto-flood
  → magic pencil: Gemini rewrites every beat · Nano Banana 2 Lite on each still
    · Gemini Omni 1.1 motion-transfers from the **source clip + twist text**
      only (never attach twisted stills — Replicate rejects video+image)
  → Telegram: Omni MP4 (`sendVideo`) + Images / Show / Copy + one `⏱` timing line
```

| Step | Law |
|---|---|
| Intake | `/igtovid` · `/pintovid` · `/pintovideo` · `/reverseprompt`. Instant: **Now Paste the link.** Then **Whats the hook?** then **Select a Prompt Model…** → **processing..** → **Workflow Started…**. Twist is not asked here. No essay. Not queued behind `/promptscan`. A stray direct `.mp4` in chat does **not** start a run unless he already typed the command. No Cancel / Retry buttons — paste a link anytime. |
| Download | `app/prompts/reverse_prompt.py::fetch_video`. Plain HTTP with a Safari UA first (Pinterest pages often carry the mp4 in JSON). Instagram login walls fall through to the logged-in stealth Chrome profile (`sources.browser_fetch`, **90s timeout** — reverse #14 hung here with no Gemini call). HLS playlists are stitched by ffmpeg (copy, then H.264 transcode if copy fails). After store, **remux HEVC/VP9 → H.264** so Gemini can read Pinterest pins (reverse #16 died with Google E001 on the raw file). Cap `PROMPT_REVERSE_MAX_VIDEO_MB` (80). Telegram forwarded files are ≤20 MB — larger clips must arrive as a link. A deploy purge can drop the Celery job: worker start re-queues unfinished reverses quietly; Telegram stays casual — paste the link again anytime. |
| Describe | After the title, Telegram asks **Gemini · GPT-6 Astra · Claude Fable 5**. **Every engine gets the video file** — we never extract stills and call that the reverse (Ashok 2026-09-11: Codex handed Astra the mp4). Gemini via Replicate (`REPLICATE_VISION_MODEL`) watches the **public `.mp4` asset URL** (Replicate fetches it — no multi-MB data-URI upload). Data-URI is the fallback when the URL cannot be fetched, or on Google **E001** after an H.264 remux. GPT-6 Astra (`gpt-6-astra`, `OPENAI_API_KEY`) tries native `input_video` / `input_file`, then Files API + `code_interpreter` with the mp4 in the container. Claude Fable 5 (`claude-fable-5`, `ANTHROPIC_API_KEY`) tries a native video block, then Files API + `container_upload` + code execution. Same system instruction + exemplar + JSON salvage/retry. **Prompt is strictly under 3000 characters including spaces** — Gemini thinking is back on (`thinking_budget=8192`, 32k output) so it plans the cut; a compress retry + deterministic `fit_prompt` is the last door. JSON is `keyword` + `prompt` + `cuts` (hard-cut start/end seconds). **`cuts` is stripped before storage** — the prompt Ashok copies never contains the array. Missing keys refuse that button and keep the other two. Stored as `vision_engine` (`gemini \| astra \| fable`); `model` is the API id that wrote the prompt. **This is the one place an AI authors a prompt** — first pass is video-only; a second pass may rewrite it against the cut-reference JPEGs (below). After that, stored and shown verbatim, never shipped half-finished. Failures say **Gemini**, never Kling's "video model failed" costume. |
| Recreate frames | After the first-pass prompt, grab **~14 JPEGs from the downloaded clip at cut timestamps** (start + end of each shot; interiors of the longest cuts if fewer than 14; never an equal-interval grid). Not the 6-frame reel storyboard. **Those JPEGs go back to the same vision model** with the video (Gemini `images[]`, max 10 — first + last cut plus a spread; Astra/Fable get the same stills after the mp4). The rewrite must match product pose, crop, lighting, camera, set, glass/liquid/hand at each labeled time. Failure or a much shorter rewrite keeps the draft. Frames are **not** re-extracted after refine, so Telegram files stay aligned with the stored prompt. Telegram does **not** auto-send them. **🖼 Images** sends **4** `sendDocument` JPEGs, then **More images ▸**. Never dump all 14. Stored as `ref_frames` JSON (migration `a3c9e1b72d04`). A grab failure keeps the prompt (`ref_error`); the row is still `done`. |
| Reel | Same composer, same 6 storyboard frames from **the source clip**, prompt scrolls verbatim. Keys `prompts/<day>/rreel-<id>-….mp4`. Telegram delivers the **original clip and the reel as `sendVideo`** (play + Save on the phone) — URL buttons alone are not enough for Instagram source clips. A reel failure keeps the clip + prompt (`reel_error`); the row is still `done`. |
...
<<<<<<< HEAD
| Magic pencil | Finished clip message is **Shall we twist the video?** with Save clip · Save reel · **Images** · Show prompt · Copy prompt · Twist. The prompt is never dumped. Show sends it; Copy sends a `.txt`. Twist starts the rewrite. Gemini (text-only second call) rewrites **every** timestamped beat — **3 tries**. If Gemini still misses (empty, short, no timestamps), **stamp the twist onto every original beat** and continue to stills + Omni — do not fail the pass on rewrite alone. **Twisted prompt stays strictly under 3000 characters including spaces**. Then each cut JPEG goes through **text+image→image** on **Nano Banana 2 Lite**. Then **Gemini Omni 1.1** (`google/gemini-omni-1.1` on Replicate) motion-transfers from the **source clip + twist text only** — never attach stills (`image` / `last_frame` / `reference_images` with `video` → Replicate ValueError). Telegram sends the Omni MP4 as `sendVideo`. Twisted stills also wait behind **Images** (4 then More). Stills stay if Omni fails (`twist_video_error`). A later fail (queue / no stills) shows **Retry twist**. Original prompt and original frames stay. Stored as `twist_text` / `twist_prompt` / `twist_frames` / `twist_video_*` (migration `c5f8a2d19e37`). A leftover Gemini tap after start **resumes** the last reverse — it does not re-ask the link. |
=======
| Magic pencil | Finished clip message is **Shall we twist the video?** with Save clip · Save reel · **Images** · Show prompt · Copy prompt · Twist. The prompt is never dumped. Show sends it; Copy sends a `.txt`. Twist starts the rewrite. Gemini (text-only second call) rewrites **every** timestamped beat — **3 tries**. If Gemini still misses (empty, short, no timestamps), **stamp the twist onto every original beat** and continue to stills + Omni — do not fail the pass on rewrite alone. **Twisted prompt stays strictly under 3000 characters including spaces**. Then each cut JPEG goes through **text+image→image** on **Nano Banana 2 Lite**. Then **Gemini Omni 1.1** (`google/gemini-omni-1.1` on Replicate) motion-transfers the original clip onto those stills. Telegram sends the Omni MP4 as `sendVideo`. Twisted stills also wait behind **Images** (4 then More). Stills stay if Omni fails (`twist_video_error`). A later fail is plain text — no Retry / Cancel buttons; paste a link or tap Twist again anytime. Original prompt and original frames stay. Stored as `twist_text` / `twist_prompt` / `twist_frames` / `twist_video_*` (migration `c5f8a2d19e37`). A leftover Gemini tap after start **resumes** the last reverse — it does not re-ask the link. |
>>>>>>> origin/main
| Pace | Worker writes `timings` JSON on `reverse_prompts`. After twisted stills, Telegram offers **Images** (4 then More) and **▶ Start the Twist**. That tap is the Omni Replicate call — never auto-start after Nano Banana. |

`/igtovid` · `/pintovid` are public. `/prompts` stays owner-only.
...
| `GET /reverse` · `GET /reverse/{id}` | Recent rows / one row: `queued \| downloading \| describing \| composing \| done \| failed`, clip + prompt + `ref_frames` + twist + Omni video + reel + `timings` |
...
- `reverse_prompts` (migration `d9e5a1c47f02`) — platform (`instagram \| pinterest \| direct \| upload`), source/media URL, stored clip, duration, **header_title**, **vision_engine** (`gemini \| astra \| fable`, migration `f2c4d6e8a910`), **keyword + prompt_text** (verbatim from the chosen model; `cuts` stripped), **ref_frames / ref_error** (migration `a3c9e1b72d04` — ~14 cut-reference JPEGs), **twist_text / twist_prompt / twist_frames / twist_status** (migration `b4e7c1a90d28` — magic pencil), **twist_video_key / twist_video_url / twist_video_error** (migration `c5f8a2d19e37` — Gemini Omni motion-transfer MP4), **timings** (migration `d6a1c4e80b29` — started_at + per-step seconds), optional `prompt_id` into the catalogue, reel key/url/error, status (`queued \| downloading \| describing \| composing \| done \| failed`).
...
| `PROMPT_TWIST_VIDEO_MODEL` | `google/gemini-omni-1.1` | After twisted stills: motion-transfer the original clip. Empty env falls back here. |
